try:
	...
	class MinerService():
		...
		def StartUp(self):
			...
			self.avgTime = 0
			self.totalUnload = 0
			self.lastStart = settings.public.ui.get("MinerLastStart%s"%cfg.eveowners.Get(session.charid).name)
			self.balls = []
			self.station = None
			...
			self.state = None
			self.location = None
			self.yieldPerLaser = self.GetYieldPerLaser()
...
##					self.state = STATE_HIBERNATE
		...
		@safetycheck
		def LoadStats(self):
			MinerStats = settings.public.ui.Get('MinerStats%s' % cfg.eveowners.Get(session.charid).name)
			if MinerStats == None:
		...
		def SaveStats(self):
			...
			settings.public.ui.Set('MinerStats%s' % cfg.eveowners.Get(session.charid).name, MinerStats)
		...
		def ClearStats(self):
			...
			settings.public.ui.Set('MinerStats%s' % cfg.eveowners.Get(session.charid).name, MinerStats)
			now = blue.os.GetTime()
			settings.public.ui.Set('MinerLastStart%s' % cfg.eveowners.Get(session.charid).name, now)
			self.lastStart = now
			self.statsTime = FormatTimeAgo(self.lastStart)
			self.UpdatePane()
		...
		def GetCargoProportion(self):
			yieldPerCycle = self.yieldPerLaser
			...
			for each in activeModules:
				(startTime, durationInMilliseconds,) = each.GetDuration()
				if durationInMilliseconds <= 0:
					portionDone = 0.0
				else:
	  				portionDone = blue.os.TimeDiffInMs(startTime) % durationInMilliseconds / durationInMilliseconds
