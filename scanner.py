try:
	...
	try:
		...
		form.Scanner.locData = dict()
		form.Scanner.curScanTarget = None
	except:
	...
	def GoToLocations(self, *args):
		...
		if uicore.uilib.Key(uiconst.VK_SHIFT):
			settings.user.ui.Set("Locations%d"%(args[0]), target)
			return
		elif uicore.uilib.Key(uiconst.VK_CONTROL):
			settings.user.ui.Set("Locations%d"%(args[0]), avg)
			return
		else:
			target = settings.user.ui.Get("Locations%d"%(args[0]))
...
##					if result.groupName == None and hasattr(result, 'groupID') and result.groupID:
##						result.groupName = cfg.invgroups.Get(result.groupID).name
...
##
##	#form.Scanner.LoadResultList = LoadResultList
	...
	def FlushDistData(self, *args):
		form.Scanner.locData.clear()
		sm.GetService('gameui').Say("Location data flushed")
	...
	def DirectionSearch(self, *args):
		...
		direction = trinity.TriVector(0.0,0.0,0.0)
	...
	form.Scanner.DirectionSearch1 = DirectionSearch
	...
	def DirectionSearch2(self, *args):
		...
		try:
			...
			itemid = stuff.itemIDs[0]
			ball = sm.GetService('michelle').GetBallpark().GetBall(itemid)
			target = Vector3(ball.x,ball.y, ball.z)
			itemName = ''
			slimItem = uix.GetBallparkRecord(itemid)
			if slimItem:
				itemName = uix.GetSlimItemName(slimItem)
			form.Scanner.curScanTarget = itemName

		except:
			sm.GetService('gameui').Say("No active item, or item has no ball")
		...
		source = Vector3(myball.x, myball.y, myball.z)

		direction = target - source
		newvec = trinity.TriVector(direction.x, direction.y, direction.z)
		newvec.Normalize()
		vec.Normalize()
		rnge = self.dir_rangeinput.GetValue()
		result = None
		try:
			result = sm.GetService('scanSvc').ConeScan(mathUtil.DegToRad(5), (rnge * 1000), newvec.x, newvec.y, newvec.z)
	...
	def DirectionSearch3(self, *args):
		from foo import Vector3
		import mathUtil
		form.Scanner.curScanTarget = None
		if ((self is None) or (self.destroyed or self.busy)):
		...
		try:
			...
			if selected is not None:
				...
				if isinstance(result.data, float):
					data = result.pos
				if not isinstance(result.data, Vector3):
					data = result.data.point

				target = Vector3(data.x, data.y, data.z)
				form.Scanner.curScanTarget = result.id
		except:
			sm.GetService('gameui').Say("Scan result acquisition error!")
	...
	form.Scanner.dscan = DirectionSearch3
	...
	def ShowResult(self, *args):
		...
		if (self.scanresult and len(self.scanresult)):
			...
			for (slimItem, ball, celestialRec,) in self.scanresult:
				...
				itemIDStr = '%s'%itemID
				if form.Scanner.curScanTarget is not None:
					itemLocRecord = None
					if itemIDStr in form.Scanner.locData.keys():
						itemLocRecord = form.Scanner.locData[itemIDStr]
					else:
						itemLocRecord = []
					if form.Scanner.curScanTarget not in itemLocRecord:
						itemLocRecord.append(form.Scanner.curScanTarget)
					form.Scanner.locData[itemIDStr] = itemLocRecord
				...
				locLabel = ''
				try:
					locLabel = '<color=0xffff3333>%s (%s)</color>'%(form.Scanner.locData[itemIDStr][-1],len(form.Scanner.locData[itemIDStr]))
				except:
					 pass
				data.label = ('%s (<color=0xff33ffff>%d</color>) %s<t>%s<t>%s' % (entryname, (itemID%1000), locLabel,
				 typeinfo.name,
				 finaldiststr))
	...
	form.Scanner.ShowDirectionalSearchResult = ShowResult
	...
	def ApplyAttributes(self, attributes):
		...
		btn = uix.GetBigButton(32, self.sr.systemTopParent, left=118)
		...
		btn.Click = self.dscan
		btn.hint = "Dscan result"
		btn.sr.icon.LoadIcon('44_05')
		self.sr.dscanBtn = btn
